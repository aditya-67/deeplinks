...
dl = deeplink.Deeplink(
    "c26506b3-a9ff-4088-a552-51b02adcd560",
    "67ae702e-eea4-4f5a-a602-f7060802f17c",
    "599228531351749724",
    mode="SANDBOX"
    )
...
def upi():
    link = dl.create_payment_link(
            amountValue=1000,
            billerBillID="Test789",
            amountExactness='EXACT'
        )

    payment_status_one = dl.check_payment_status(
        platformBillID=link['data']['platformBillID'])

    trigger_payment = dl.trigger_mock_payment(
        amountValue=500,
        upiID=link['data']['paymentLink']['upiID'])

    payment_status_two = dl.check_payment_status(
        platformBillID=link['data']['platformBillID'])

    return render_template("upi.html", paymentLink = json.dumps(link, indent=4), paymentStatusOne= json.dumps(payment_status_one, indent=4), triggerPayment = trigger_payment, paymentStatusTwo= json.dumps(payment_status_two, indent=4))
